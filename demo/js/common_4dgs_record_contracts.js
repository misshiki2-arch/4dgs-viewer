...
export const WEBGPU_DEPTH_SORT_COMPARISON_COMPUTE_MODE =
...
export const WEBGPU_TILE_COMPOSITE_ACCUMULATION_DRY_RUN_COMPARISON_COMPUTE_MODE =
...
export const WEBGPU_CONSTRAINED_DISPLAY_ADAPTER_DRY_RUN_COMPARISON_COMPUTE_MODE =
...
export const WEBGPU_VIEWER_CANVAS_CURRENT_TEXTURE_PATH_COMPUTE_MODE =
...
export const WEBGPU_BACKEND_FRAME_LIFECYCLE_PROTOTYPE_COMPUTE_MODE =
  'webgpu-backend-frame-lifecycle-prototype';

export const WEBGPU_BACKEND_FRAME_CONTROLLED_REPEATED_EXECUTION_COMPUTE_MODE =
  'webgpu-backend-frame-controlled-repeated-execution';

export const WEBGPU_BACKEND_VIEWER_LOOP_ADAPTER_COMPUTE_MODE =
  'webgpu-backend-viewer-loop-adapter';

export const WEBGPU_BACKEND_VIEWER_LIFECYCLE_INTEGRATION_COMPUTE_MODE =
  'webgpu-backend-viewer-lifecycle-integration-boundary';

export const WEBGPU_BACKEND_VIEWER_LIFECYCLE_CONTROLLED_EXECUTION_COMPUTE_MODE =
  'webgpu-backend-viewer-lifecycle-controlled-execution';

export const WEBGPU_BACKEND_VIEWER_FRAME_EXECUTOR_COMPUTE_MODE =
...
export const WEBGPU_CAMERA_AWARE_VISIBLE_OUTPUT_COMPUTE_MODE =
  'camera-control-scheduler-aware-visible-webgpu-output';
...
export const WEBGPU_VISIBLE_RECORD_FIELD_COMPUTE_MODES = Object.freeze({
  ...
  webgpuViewerCanvasNativeBoundedColorSamples:
    WEBGPU_VIEWER_CANVAS_NATIVE_BOUNDED_COLOR_SAMPLES_COMPUTE_MODE,
  ...
  webgpuBackendFrameLifecyclePrototype:
    WEBGPU_BACKEND_FRAME_LIFECYCLE_PROTOTYPE_COMPUTE_MODE,
  webgpuBackendFrameControlledRepeatedExecution:
    WEBGPU_BACKEND_FRAME_CONTROLLED_REPEATED_EXECUTION_COMPUTE_MODE,
  webgpuBackendViewerLoopAdapter:
    WEBGPU_BACKEND_VIEWER_LOOP_ADAPTER_COMPUTE_MODE,
  webgpuBackendViewerLifecycleIntegrationBoundary:
    WEBGPU_BACKEND_VIEWER_LIFECYCLE_INTEGRATION_COMPUTE_MODE,
  webgpuBackendViewerLifecycleControlledExecution:
    WEBGPU_BACKEND_VIEWER_LIFECYCLE_CONTROLLED_EXECUTION_COMPUTE_MODE,
  ...
  webgpuCameraAwareVisibleOutput:
    WEBGPU_CAMERA_AWARE_VISIBLE_OUTPUT_COMPUTE_MODE,
...
export function buildWebGpuTileListCompositorContract({
  ...
  gpuSidePerTileSortReady = false,
  boundedPerTileSortUsed = false,
  depthKeyBufferConsumed = false,
  depthSortedOrderedReferencesGenerated = false,
  depthSortedReferencesConsumedByAccumulation = false,
  sortedAccumulationPathUsed = false,
  sortDispatchCount = 0,
  sortWorkItemCount = 0,
  sortedTileCount = 0,
  ...
  fixedReferenceCameraActivationMode = null,
  ...
  cameraConstantsRoutingReady = false,
  viewMatrixSource = null,
  projectionMatrixSource = null,
  viewportSource = null,
  screenSpaceConventionSource = null,
  backgroundPolicySource = null,
  ...
  const perTileSortReady =
    ...
    depthKeyBufferConsumed === true &&
    depthSortedOrderedReferencesGenerated === true &&
    depthSortedReferencesConsumedByAccumulation === true &&
    sortedAccumulationPathUsed === true &&
    sortDispatchCount > 0 &&
    sortWorkItemCount > 0 &&
    sortedTileCount > 0 &&
    sortedReferenceCount > 0 &&
  ...
  const effectiveStep107DesignGatePreserved =
    ...
    fixedReferenceCameraContractGateReady === true;
  const effectiveFixedReferenceCameraActivationMode =
  ...
  const effectiveViewMatrixSource =
    viewMatrixSource ?? viewMatrixEvidence?.source ?? null;
  const effectiveProjectionMatrixSource =
    projectionMatrixSource ?? projectionMatrixEvidence?.source ?? null;
  const effectiveViewportSource =
    viewportSource ??
    cudaReferenceCameraEvidenceSources?.viewport ??
    (
      viewportEvidence?.ready === true
        ? 'viewer-canvas-output-viewport'
        : null
    );
  const effectiveScreenSpaceConventionSource =
    screenSpaceConventionSource ??
    screenSpaceConventionEvidence?.viewMatrixSource ??
    cudaReferenceCameraEvidenceSources?.screenSpaceConvention ??
    null;
  const effectiveBackgroundPolicySource =
    backgroundPolicySource ??
    backgroundPolicyEvidence?.backgroundPolicy ??
    cudaReferenceCameraEvidenceSources?.background ??
    null;
  const effectiveWebgpuCameraConstantsSource =
    webgpuCameraConstantsSource ??
    effectiveWebgpuCameraConstantsContract.source;
  const effectiveCameraConstantsRoutingReady =
    cameraConstantsRoutingReady === true ||
    (
      effectiveFixedReferenceCameraActivationReady === true &&
      effectiveCudaReferenceCameraEvidenceReady === true &&
      typeof effectiveWebgpuCameraConstantsSource === 'string' &&
      effectiveWebgpuCameraConstantsSource.includes('cuda-reference') &&
      typeof effectiveViewMatrixSource === 'string' &&
      typeof effectiveProjectionMatrixSource === 'string' &&
      typeof effectiveViewportSource === 'string' &&
      typeof effectiveScreenSpaceConventionSource === 'string' &&
      typeof effectiveBackgroundPolicySource === 'string'
    );
  const effectiveStep108CameraEvidencePreserved =
    step108CameraEvidencePreserved === true ||
    (
      effectiveCudaReferenceCameraEvidenceReady === true &&
  ...
  return {
    ...
    step91ProductionAccumulationMode,
    gpuSidePerTileSortReady: perTileSortReady,
    boundedPerTileSortUsed,
    depthKeyBufferConsumed,
    ...
    fixedReferenceCameraActivationReady:
      effectiveFixedReferenceCameraActivationReady,
    fixedReferenceCameraActivationMode:
      effectiveFixedReferenceCameraActivationMode,
    fixedReferenceCameraActivationBlockedReason:
    ...
    backgroundPolicyEvidence,
    webgpuCameraConstantsSource:
      effectiveWebgpuCameraConstantsSource,
    cameraConstantsRoutingReady: effectiveCameraConstantsRoutingReady,
    viewMatrixSource: effectiveViewMatrixSource,
    projectionMatrixSource: effectiveProjectionMatrixSource,
    viewportSource: effectiveViewportSource,
    screenSpaceConventionSource: effectiveScreenSpaceConventionSource,
    backgroundPolicySource: effectiveBackgroundPolicySource,
    ...
    step108CameraEvidencePreserved:
      effectiveStep108CameraEvidencePreserved,
    step109NextStepRecommendedGoal:
      step109NextStepRecommendedGoal ??
      (
        effectiveCameraConstantsRoutingReady &&
        visualParityComparisonAllowed === true

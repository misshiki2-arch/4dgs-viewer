...
function buildGpuScreenStateLines(gpuScreenSummary) {
  ...
  pushLine(lines, 'gpuScreenSharedMergePolicySelectedPath', gpuScreenSummary.gpuScreenSharedMergePolicySelectedPath ?? 'none');
  pushLine(lines, 'gpuScreenSharedMergePolicyReason', gpuScreenSummary.gpuScreenSharedMergePolicyReason ?? 'none');
...
function buildTransformBatchLines(transformBatchSummary) {
  ...
  pushLine(lines, 'transformBatchPlannerMode', transformBatchSummary.plannerMode ?? 'planner-mode-none');
  pushLine(lines, 'transformBatchPlannerReason', transformBatchSummary.plannerReason ?? 'planner-reason-none');
  pushLine(lines, 'transformBatchPlannerCoupledToFramePolicy', fmtBool(!!transformBatchSummary.plannerCoupledToFramePolicy));
  pushLine(lines, 'transformBatchPolicyAtlasPlanningMode', transformBatchSummary.policyAtlasPlanningMode ?? 'none');
  pushLine(lines, 'transformBatchPolicyAtlasPlanningReason', transformBatchSummary.policyAtlasPlanningReason ?? 'none');
  pushLine(lines, 'transformBatchAtlasPressureMode', transformBatchSummary.atlasPressureMode ?? 'atlas-pressure-no-history');
  pushLine(lines, 'transformBatchAtlasPressureReason', transformBatchSummary.atlasPressureReason ?? 'atlas-pressure-no-backend-atlas-history');
  pushLine(lines, 'transformBatchAtlasPressureAllocationBytes', fmtInt(transformBatchSummary.atlasPressureAllocationBytes));
  pushLine(lines, 'transformBatchAtlasPressureSavedAllocationBytes', fmtInt(transformBatchSummary.atlasPressureSavedAllocationBytes));
  pushLine(lines, 'transformBatchAtlasPressureChurnReason', transformBatchSummary.atlasPressureChurnReason ?? 'none');
  pushLine(lines, 'transformBatchAtlasPressureReusePreferred', fmtBool(!!transformBatchSummary.atlasPressureReusePreferred));
  pushLine(lines, 'transformBatchAtlasPressureRebuildAllowed', fmtBool(!!transformBatchSummary.atlasPressureRebuildAllowed));
...
function buildTransformThroughputLines(transformThroughputSummary) {
  ...
  pushLine(lines, 'transformThroughputPlannerMode', transformThroughputSummary.plannerMode ?? 'planner-mode-none');
  pushLine(lines, 'transformThroughputPlannerReason', transformThroughputSummary.plannerReason ?? 'planner-reason-none');
  pushLine(lines, 'transformThroughputPlannerCoupledToFramePolicy', fmtBool(!!transformThroughputSummary.plannerCoupledToFramePolicy));
  pushLine(lines, 'transformThroughputPolicyAtlasPlanningMode', transformThroughputSummary.policyAtlasPlanningMode ?? 'none');
  pushLine(lines, 'transformThroughputPolicyAtlasPlanningReason', transformThroughputSummary.policyAtlasPlanningReason ?? 'none');
  pushLine(lines, 'transformThroughputAtlasPressureMode', transformThroughputSummary.atlasPressureMode ?? 'atlas-pressure-no-history');
  pushLine(lines, 'transformThroughputAtlasPressureReason', transformThroughputSummary.atlasPressureReason ?? 'atlas-pressure-no-backend-atlas-history');
  pushLine(lines, 'transformThroughputAtlasPressureAllocationBytes', fmtInt(transformThroughputSummary.atlasPressureAllocationBytes));
  pushLine(lines, 'transformThroughputAtlasPressureSavedAllocationBytes', fmtInt(transformThroughputSummary.atlasPressureSavedAllocationBytes));
  pushLine(lines, 'transformThroughputAtlasPressureChurnReason', transformThroughputSummary.atlasPressureChurnReason ?? 'none');
  pushLine(lines, 'transformThroughputAtlasPressureReusePreferred', fmtBool(!!transformThroughputSummary.atlasPressureReusePreferred));
  pushLine(lines, 'transformThroughputAtlasPressureRebuildAllowed', fmtBool(!!transformThroughputSummary.atlasPressureRebuildAllowed));
...
function buildDrawThroughputLines(drawThroughputSummary) {
  ...
  pushLine(lines, 'drawThroughputSharedMergePolicySelectedPath', drawThroughputSummary.sharedMergePolicySelectedPath ?? 'none');
  pushLine(lines, 'drawThroughputSharedMergePolicyReason', drawThroughputSummary.sharedMergePolicyReason ?? 'none');
...
function buildFrameGpuThroughputLines(frameGpuThroughputSummary) {
  ...
  pushLine(lines, 'gpuFrameTransformPlannerMode', frameGpuThroughputSummary.transformPlannerMode ?? 'planner-mode-none');
  pushLine(lines, 'gpuFrameTransformPlannerReason', frameGpuThroughputSummary.transformPlannerReason ?? 'planner-reason-none');
  pushLine(lines, 'gpuFrameTransformPlannerCoupledToFramePolicy', fmtBool(!!frameGpuThroughputSummary.transformPlannerCoupledToFramePolicy));
  pushLine(lines, 'gpuFrameTransformAtlasPressureMode', frameGpuThroughputSummary.transformAtlasPressureMode ?? 'atlas-pressure-no-history');
  pushLine(lines, 'gpuFrameTransformAtlasPressureReason', frameGpuThroughputSummary.transformAtlasPressureReason ?? 'atlas-pressure-no-backend-atlas-history');
  pushLine(lines, 'gpuFrameTransformAtlasPressureAllocationBytes', fmtInt(frameGpuThroughputSummary.transformAtlasPressureAllocationBytes));
  pushLine(lines, 'gpuFrameTransformAtlasPressureSavedAllocationBytes', fmtInt(frameGpuThroughputSummary.transformAtlasPressureSavedAllocationBytes));
  pushLine(lines, 'gpuFrameTransformAtlasPressureChurnReason', frameGpuThroughputSummary.transformAtlasPressureChurnReason ?? 'none');
  ...
  pushLine(lines, 'gpuFrameDrawMergePolicySelectedPath', frameGpuThroughputSummary.drawMergePolicySelectedPath ?? 'none');
  ...
  pushLine(lines, 'gpuFrameTransformPolicyAtlasPlanningMode', frameGpuThroughputSummary.transformPolicyAtlasPlanningMode ?? 'none');
  pushLine(lines, 'gpuFrameTransformPolicyAtlasPlanningReason', frameGpuThroughputSummary.transformPolicyAtlasPlanningReason ?? 'none');
...
export function buildPackedLines(buildStats, drawPathSelection, drawStats) {
  return [
    ...
    `packedDirectSharedMergePolicySelectedPath=${drawStats?.packedDirectSharedMergePolicySelectedPath ?? 'none'}`,
    `packedDirectSharedMergePolicyReason=${drawStats?.packedDirectSharedMergePolicyReason ?? 'none'}`,

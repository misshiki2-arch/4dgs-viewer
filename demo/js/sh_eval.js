...
function coeffVec(raw, i, k) {
  ...
  if (k === 0) return [raw.f_dc[base0] || 0, raw.f_dc[base0 + 1] || 0, raw.f_dc[base0 + 2] || 0];
  const j = (k - 1) * 3;
  return [raw.f_rest[restBase + j] || 0, raw.f_rest[restBase + j + 1] || 0, raw.f_rest[restBase + j + 2] || 0];
}
...
export function evalSHColor(raw, i, camPos, pos, timestamp, timeDuration, useSH, forceSh3d) {
  if (!useSH) {
    return [
      clamp((raw.f_dc[i * raw.fdcDim] || 0) + 0.5, 0, 1),
  ...
  const dir = new THREE.Vector3(pos[0] - camPos.x, pos[1] - camPos.y, pos[2] - camPos.z);
    ...
    if (deg > 1) {
      ...
      if (deg > 2) {
        ...
        if (gaussianDim4 && !forceSh3d && degT > 0) {
          ...
          const terms = [l0m0,l1m1,l1m0,l1p1,l2m2,l2m1,l2m0,l2p1,l2p2,l3m3,l3m2,l3m1,l3m0,l3p1,l3p2,l3p3];
          const t1 = Math.cos(2 * Math.PI * dir_t / Math.max(1e-6, timeDuration));
          for (let k = 0; k < terms.length; k++) addScaled(result, coeffVec(raw, i, 16 + k), t1 * terms[k]);
          if (degT > 1) {
            const t2 = Math.cos(4 * Math.PI * dir_t / Math.max(1e-6, timeDuration));
            for (let k = 0; k < terms.length; k++) addScaled(result, coeffVec(raw, i, 32 + k), t2 * terms[k]);
          }
        }
      }
    }

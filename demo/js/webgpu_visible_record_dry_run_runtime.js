...
function readWebGpuRenderAttribute(renderAttributes, recordIndex) {
  const base = recordIndex * 8;
  if (!renderAttributes || base + 7 >= renderAttributes.length) return null;
  const sourceCode = Number(renderAttributes[base + 6]);
  if (sourceCode !== 81) return null;
  const radiusPx = Number(renderAttributes[base + 0]);
  const alpha = Number(renderAttributes[base + 1]);
  const r = Number(renderAttributes[base + 2]);
  const g = Number(renderAttributes[base + 3]);
  const b = Number(renderAttributes[base + 4]);
  const temporalWeight = Number(renderAttributes[base + 5]);
  if (![radiusPx, alpha, r, g, b].every(Number.isFinite)) return null;
  return {
    radiusPx,
    temporalWeight: Number.isFinite(temporalWeight) ? temporalWeight : null,
    colorAlpha: {
...
function buildCameraAwareVisibleOutputSamples({
  ...
  canvasWidth,
  canvasHeight,
  visibleRecordGateSummary = null,
  maxSampleCount = 192,
  outputPointRadiusPx = 9
}) {
  const rawPositionRepairRecordCount = Number(
    visibleRecordGateSummary?.rawPositionRepairRecordCount ?? 0
  );
  const rawPositionRepairUsed = rawPositionRepairRecordCount > 0;
  const visibleRecordPathClassification =
    visibleRecordGateSummary?.trueVisibleRecordPathClassification ??
    ...
    return {
      ...
      contract: buildUnavailableCameraAwareVisibleOutputContract(
        'webgpu-visible-record-output-unavailable',
        {
          ...
          visibleRecordPathClassification,
          rawPositionRepairUsed,
          full4DStateDrivenTrueNative: false,
  ...
  const computedPointRadius =
    webgpuGaussianAttributeEvaluation?.normalBackendPointRadiusPx;
  const resolvedOutputPointRadiusPx =
    Number.isFinite(computedPointRadius) && computedPointRadius > 0
      ? computedPointRadius
      : outputPointRadiusPx;
  ...
  const contract = buildCameraAwareVisibleOutputContract({
    ...
    webgpuComputedAttributeFields:
      webgpuGaussianAttributeEvaluation?.computedAttributeFields ?? [],
    renderAttributeClassification:
      webgpuGaussianAttributeEvaluation?.renderAttributeClassification ?? null,
...
function buildStatePositionsForCandidates(raw, candidateIndices, buildConfig) {
  ...
  let baselineStatePositionCount = 0;
  let unavailableStatePositionCount = 0;
  let usedCuda4DStateHelper = false;
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    if (state?.pos) {
      ...
      usedCuda4DStateHelper =
        usedCuda4DStateHelper || state.usedCuda4DStateHelper === true;
      helperVersion = helperVersion ?? state.helperVersion ?? null;
    } else {
      const xyzBase = srcIndex * raw.xyzDim;
      const x = Number(raw.xyz?.[xyzBase + 0]);
      const y = Number(raw.xyz?.[xyzBase + 1]);
      const z = Number(raw.xyz?.[xyzBase + 2]);
      if (Number.isFinite(x) && Number.isFinite(y) && Number.isFinite(z)) {
        out[o + 0] = x;
        out[o + 1] = y;
        out[o + 2] = z;
        out[o + 3] = 0.75;
        baselineStatePositionCount += 1;
      } else {
        out[o + 0] = 0;
        out[o + 1] = 0;
        out[o + 2] = 0;
    ...
    contract: buildWebGpu4DStateSourceContract({
      stateSourceMode:
        baselineStatePositionCount > 0
          ? 'cpu-materialized-4d-state-source-with-raw-baseline-position'
          : 'cpu-materialized-4d-state-source',
      ...
      rawXyzRepairInVisibleRecordComputeRequired: unavailableStatePositionCount > 0,
      reason:
        computed4DStatePositionCount > 0
          ? null
          : '4D conditional state evaluation culled this candidate batch; Step79 supplies a minimal baseline state source so visible-record compute consumes statePositions instead of raw repair'
    })
  };
}

function summarizeStatePositionAvailability(statePositions) {
  const count = Math.floor((statePositions?.length ?? 0) / 4);
  let availableCount = 0;
  for (let i = 0; i < count; i += 1) {
    if (Number(statePositions[i * 4 + 3]) > 0.5) availableCount += 1;
  }
  return {
    statePositionCount: count,
    statePositionAvailableCount: availableCount,
    statePositionUnavailableCount: Math.max(0, count - availableCount),
    statePositionAvailabilityRatio:
      count > 0 ? availableCount / Math.max(1, count) : null
  };
}

function summarizeWebGpuVisibleRecordGate(records, recordCount) {
  ...
  for (let row = 0; row < recordCount; row += 1) {
    const base = row * RECORD_FLOATS;
    if (Number(records?.[base + 1]) > 0.5) {
      validRecordCount += 1;
      const stateSourceCode = Number(records?.[base + 10]);
  ...
  return {
    ...
    scaffoldNote:
      phaseStep === 'phase3-step105'
        ...
        : phaseStep === 'phase3-step102'
        ? 'Phase 3 Step102 connects selective dirty execution to persistent WebGPU production resource lifecycle, reuse policy, and realtime bottleneck evidence.'
    ...
    recordCount: cpuReference.count,
    validRecordCount: computeResult.validRecordCount,
    cpuReferenceValidRecordCount: cpuReference.validCount,
    webgpu4DStateSourceContract: webgpu4DStateSource.contract,
    webgpuGaussianAttributeEvaluationContract:
      webgpu4DStateSource.gaussianAttributeEvaluationContract,
    webgpuGaussianFootprintEvaluationContract:
    ...
    webgpuVisibleRecordGateSummary: {
      ...
      renderAttributeClassification:
        webgpu4DStateSource.gaussianAttributeEvaluationContract
          ?.renderAttributeClassification ?? null,
      renderPayloadClassification:
        webgpu4DStateSource.gaussianAttributeEvaluationContract
      ...
      baselineStatePositionCount:
        webgpu4DStateSource.contract?.baselineStatePositionCount ?? 0,
      unavailableStatePositionCount:
        webgpu4DStateSource.contract?.unavailableStatePositionCount ?? 0,
      validRecordCount: computeResult.validRecordCount,
      statePositionRecordCount: computeResult.statePositionRecordCount,
      full4DStatePositionRecordCount:
        computeResult.full4DStatePositionRecordCount,
      ...
      stateSourceBaselineRecordCount:
        computeResult.stateSourceBaselineRecordCount,
      rawPositionRepairRecordCount: computeResult.rawPositionRepairRecordCount,
      projectionGatePassedCount: computeResult.projectionGatePassedCount,
      visibilityGateMode: computeResult.visibilityGateMode,
      trueVisibleRecordPathReady: computeResult.trueVisibleRecordPathReady,
      trueVisibleRecordPathClassification:
        computeResult.trueVisibleRecordPathClassification,
